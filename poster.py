...
if __name__ == '__main__':
    ...
    for file in os.listdir('./stock_data/'):
        if file.endswith('.csv'):
            ...
            data['Three White Soldiers'] = np.where((data['Day Close'] == 'Up') & (data['Open'] > data['Open'].shift(-1)) & (data['Open'] < data['Close'].shift(-1)) & (data['Day Close'].shift(-1) == 'Up') & (data['Open'].shift(-1) > data['Open'].shift(-2)) & (data['Open'].shift(-1) < data['Close'].shift(-2)), True, False)
